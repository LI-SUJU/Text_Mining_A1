...
# # data_train = fetch_20newsgroups(
# #     subset="train", categories=categories_test, shuffle=True, random_state=42
# # )
...
# # )
# # plt.savefig("score_training_trade_off.png")
...
# # # save ax1 as score_training_trade_off.png, ax2 as score_trade_off.png

# # # if file exists, it will be overwritten
# # plt.savefig("score_testing_trade_off.png")
...
# Store the results
results = {name: [] for name in vectorizers.keys()}

# Perform 10 competitions
for i in range(10):
    print(f"Competition {i+1}")
    for vectorizer_name, vectorizer in vectorizers.items():
        print(f"Using {vectorizer_name} features")
        X_train, X_test, y_train, y_test, feature_names = load_and_vectorize(vectorizer)
        clf = ComplementNB(alpha=0.1)
        score = cross_validate_model(clf, X_train, y_train)
        results[vectorizer_name].append(score)

# Calculate the average score for each vectorizer
average_scores = {name: np.mean(scores) for name, scores in results.items()}

# Plot the results
fig, ax = plt.subplots(figsize=(10, 6))
vectorizer_names = list(average_scores.keys())
average_scores_values = list(average_scores.values())
for i, v in enumerate(average_scores_values):
    ax.text(i, v + 0.01, f"{v:.2f}", ha='center', va='bottom')
# Adjust the figure size and layout for a more compact view
# fig.set_size_inches(8, 5)
# plt.tight_layout()
bar_width = 0.3
ax.bar(vectorizer_names, average_scores_values, color=['blue', 'green', 'red'], alpha=0.6, width=bar_width)
ax.set_xlabel('Vectorizer')
ax.set_ylabel('Average Cross-Validation Score')
ax.set_title('Average Cross-Validation Score for Different Vectorizers with Complement Naive Bayes')

plt.ylim(0, 1)
plt.savefig("average_cross_validation_scores.png")
